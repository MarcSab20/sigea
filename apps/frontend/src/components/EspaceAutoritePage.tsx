// apps/frontend/src/components/EspaceAutoritePage.tsx
...
export interface EspaceAutoriteProps {
  /** Discriminant transmis au serveur et affiché dans les libellés. */
  autorite:   'CEMAA' | 'MAGE';
  titre:      string;
  sousTitre:  string;
  /** Préfixe des routes API : '/cemaa' ou '/mage'. */
  prefixeApi: string;
  /** Teinte dominante — DOIT différer d'un espace à l'autre (voir MagePage). */
  accent:     'red' | 'amber';
  /** Glyphe d'en-tête. */
  sigle:      string;
}

export default function EspaceAutoritePage({
  autorite, titre, sousTitre, prefixeApi, accent, sigle,
}: EspaceAutoriteProps): React.ReactElement {
  ...
  const handleEnvoyerConsigne = async (): Promise<void> => {
    ...
    try {
      ...
      await api.post(`${prefixeApi}/consignes`, {
        vol_id: volId,
      ...
      toast.success('Consigne {autorite} transmise — préremplissage forcé activé dans le manifeste');
    ...
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ padding: '10px 16px', background: T.redBg,
          ...
          {sigle} Zone réservée {autorite} · Les éléments injectés sont automatiquement intégrés
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {typeConsigne === 'PERSONNEL' && (
            <Card style={{ padding: '18px 22px',
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: T.text, marginBottom: 14,
                ...
                Injection de personnel — Consigne {autorite}
              </div>
              <div style={{ padding: '8px 12px', background: T.amberBg,
                ...
                ⚠ Le personnel injecté via consigne {autorite} est automatiquement verrouillé dans
          ...
          {typeConsigne === 'MATERIEL' && (
            <Card style={{ padding: '18px 22px', border: `2px solid ${T.redBorder}` }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: T.text, marginBottom: 14,
                display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 16 }}>📦</span>
                Injection de matériel — Consigne {autorite}
              </div>
          ...
          <button onClick={handleEnvoyerConsigne} disabled={!volId || submitting} style={{
            ...
            {submitting ? 'Transmission…' : '{sigle} Émettre la consigne {autorite}'}
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Card style={{ padding: '16px 18px', background: T.redBg,
            ...
            <div style={{ fontSize: 11, color: T.textSub, lineHeight: 1.8 }}>
              ...
              {sigle} Éléments sensibles : chiffrement AES-256<br />
              {sigle} Masqués aux acteurs non-{autorite}
            </div>
          </Card>
